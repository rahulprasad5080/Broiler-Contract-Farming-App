  ...
  const exportBatchReport = async (format: "excel" | "pdf") => {
    ...
    try {
      ...
      const downloaded = await triggerWebDownload(
        response,
        `batch-report.${format === "excel" ? "xlsx" : "pdf"}`,
      );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {canViewOverview ? (
          ...
          <View style={styles.panel}>
            ...
            <Text style={styles.panelSubtitle}>
              Summary KPIs and exports are limited to owner and supervisor accounts. Batch and farm
              summaries below use the same live backend report APIs available to farmers.
            </Text>
          </View>
        )}
        ...
        <View style={styles.panel}>
          ...
          <Text style={styles.panelSubtitle}>
            Enter a batch ID to load live batch performance
            {canExport ? " and exports." : "."}
          </Text>
          ...
          {batchSummary ? (
            <View style={styles.detailCard}>
              ...
              <DetailRow
                ...
                value={
                  batchSummary.mortalityRate !== null && batchSummary.mortalityRate !== undefined
                    ? `${Number(batchSummary.mortalityRate).toFixed(2)}%`
                    : "0.00%"
                }
              />
              <DetailRow
                label="FCR"
                value={
                  batchSummary.fcr !== null && batchSummary.fcr !== undefined
                    ? Number(batchSummary.fcr).toFixed(2)
                    : "0.00"
                }
              />
          ...
          {farmSummary ? (
            <View style={styles.detailCard}>
              ...
              <DetailRow
                label="Average FCR"
                value={
                  farmSummary.averageFcr !== null && farmSummary.averageFcr !== undefined
                    ? Number(farmSummary.averageFcr).toFixed(2)
                    : "0.00"
                }
              />
            </View>
